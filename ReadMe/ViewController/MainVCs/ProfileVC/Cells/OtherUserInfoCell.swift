...
import UIKit
class OtherUserInfoCell: UICollectionReusableView {
    
    let profileImgView:UIImageView = {
        let iv = UIImageView(image: R.image.profileImgGirl())
    ...
    let sendMsgBtn:UIButton = {
        ...
        b.setTitleColor(.primaryDisbledTextColor, for: .normal)
        b.layer.borderWidth = 1
        b.layer.borderColor = #colorLiteral(red: 0.9450980392, green: 0.4509803922, blue: 0.4901960784, alpha: 1)
        b.withSize(CGSize(width: 109, height: 39))
        b.layer.cornerRadius = 19.5
        b.titleLabel?.font = R.font.poppinsMedium(size: 14)
        return b
...
//        stack(hstack(UIView(),profileImgView,UIView(),distribution:.equalCentering),idLbl,followStack,mainBioLbl,editProfileBtn,tabs, spacing: 16, alignment: .fill, distribution: .fill).padTop(16).padBottom(16).padLeft(16).padRight(16)
        ...
        let btnStack = UIStackView(arrangedSubviews: [followProfileBtn,sendMsgBtn])
        ...
        addSubview(btnStack)
        btnStack.centerHorizontally()
        btnStack.bottomAnchor.constraint(equalTo: tabs.topAnchor, constant: -24).isActive = true
        addSubview(mainBioLbl)

        mainBioLbl.anchor(top: followStack.bottomAnchor, leading: leadingAnchor, bottom: btnStack.topAnchor, trailing: trailingAnchor,padding: UIEdgeInsets(top: 16, left: 16, bottom: 16, right: 16))

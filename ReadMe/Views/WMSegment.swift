...
open class WMSegment: UIControl {
    ...
    public var bottomBarframeScale:CGFloat = 1.0{
        didSet{
            updateView()
        }
    }
    ...
    fileprivate var selectorWidth:CGFloat = 0.0
 //   {
//        didSet{
//            updateView()
//        }
//    }
    public var dividerHeightScale:CGFloat = 0.5{
        didSet{
            updateView()
        }
    }
    ...
    public var dividerBackgroundColor:UIColor = .lightGray
    ...
    public var isEnableDivider:Bool=false{
        didSet{
            if isEnableDivider{
                updateView()
            }
    ...
    func setupDivider(){
        ...
        for index in 0...titles.count - 1{
            let startPosition = frame.width/CGFloat(titles.count)*CGFloat(index+1)
            let divider = UIView(frame: CGRect(x: startPosition, y: frame.height*(1-dividerHeightScale)/2, width: dividerWidth, height: dividerHeightScale*frame.height))
            divider.backgroundColor = dividerBackgroundColor
            addSubview(divider)
    ...
    func setupSelector() {
        ...
        selectorWidth = bottomBarframeScale*frame.width / CGFloat(titles.count)
        if self.type == .onlyImage {
            selectorWidth = frame.width / CGFloat(images.count)
        }
        
        let startPosition:CGFloat = 0.5*frame.width*(1-bottomBarframeScale)/CGFloat(titles.count)
        if selectorType == .normal {
            selector = UIView(frame: CGRect(x:startPosition, y: 0, width: selectorWidth, height: frame.height))
            if isRounded {
            ...
            print("fram",frame.width)
            selector = UIView(frame: CGRect(x:startPosition, y: frame.height - bottomBarHeight, width: selectorWidth, height: bottomBarHeight))
            selector.layer.cornerRadius = 0
        }
    ...
    @objc func buttonTapped(_ sender: UIButton) {
        ...
        for (buttonIndex, btn) in buttons.enumerated() {
            ...
            if btn == sender {
                ...
                let startPosition = selectorWidth*1/bottomBarframeScale * CGFloat(buttonIndex) + 0.5*selectorWidth*(1-bottomBarframeScale)/bottomBarframeScale
                if self.animate {
    ...
    open func setSelectedIndex(_ index: Int) {
        for (buttonIndex, btn) in buttons.enumerated() {
            ...
            if btn.tag == index {
                selectedSegmentIndex = buttonIndex
                let startPosition = selectorWidth * CGFloat(buttonIndex)
                if self.animate {
